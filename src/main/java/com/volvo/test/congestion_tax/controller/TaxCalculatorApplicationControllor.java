package com.volvo.test.congestion_tax.controller;

import com.volvo.test.congestion_tax.core.CongestionTaxCalculator;
import com.volvo.test.congestion_tax.model.TaxCalculatorRequest;
import com.volvo.test.congestion_tax.model.TaxCalculatorResponse;
...
public class TaxCalculatorApplicationControllor {
    ...
    @Operation(summary = "Calculate the tax", description = "This service for calculating the tax in Gothenburg city of Sweden")
    public ResponseEntity<TaxCalculatorResponse> taxCalculate(
            @ApiParam(value = "Request for Tax Calculation, Vehicle Type and list of dates and Date format is : yyyy-MM-dd HH:mm:ss", required = true)
            @RequestBody TaxCalculatorRequest request) {
